...
class MainMenu(object):
    ...
    def initOneButton(self, node, size, pos, href, swapHref, clickMethod, angle=0):
        ...
        node.pivot=(0,0)
        node.angle=angle
        node.setEventHandler(avg.CURSORDOWN, avg.TOUCH or avg.MOUSE, clickMethod)
        node.setEventHandler(avg.CURSOROVER, avg.TOUCH or avg.MOUSE, lambda event : self.glow(event, node));
        node.setEventHandler(avg.CURSOROUT, avg.TOUCH or avg.MOUSE, lambda event : self.glow(event, node));
    ...
    def createPointMenuButtons(self, parentNode):
        ...
        """
        self.back.setEventHandler(avg.CURSORDOWN, avg.TOUCH or avg.MOUSE, lambda event: self.fromPointMenu(event, parentNode))
    ...
    def createTimeMenuButtons(self, parentNode):
        ...
        """
        self.back.setEventHandler(avg.CURSORDOWN, avg.TOUCH or avg.MOUSE, lambda event: self.fromTimeMenu(event,parentNode))
    ...
    def createHelpMenuButtons(self, helpDiv):
        ...
        """
        self.back.setEventHandler(avg.CURSORDOWN, avg.TOUCH or avg.MOUSE, lambda event: self.fromHelpMenu(event))
        self.letBackButtonAppear()
    ...
    def introGame(self):
        ...
        leftToRight = avg.WordsNode(font="DejaVu Sans", variant="Bold", fontsize=util.convertFontSize(32), color="FFFFFF", text = "Prepare for Fight!" , parent=parent)
        leftToRight.pos = (-leftToRight.getMediaSize()[0], util.height//32)
        
        rightToLeft = avg.WordsNode(font="DejaVu Sans", variant="Bold", fontsize=util.convertFontSize(32), color="FFFFFF", text = "Prepare for Fight!", pos = (util.width, util.height // 16*15), parent=parent)
        
        self.time = avg.WordsNode(font="DejaVu Sans", variant="Bold", fontsize=util.convertFontSize(64), color="FFFFFF", text ="10", parent=parent)
        self.time.pos = ((util.width-self.time.getMediaSize()[0])//2, (util.height -self.time.getMediaSize()[1])//2)

        self.i = 10
        self.timerX = self.player.setInterval(1000, self.timer)
                
        avg.LinearAnim(leftToRight, "pos", 10000, leftToRight.pos, (util.width, leftToRight.pos.y)).start()
        avg.LinearAnim(rightToLeft, "pos", 10000, rightToLeft.pos, (-leftToRight.getMediaSize()[0], rightToLeft.pos.y)).start()
    ...
    def pointClick(self,event,points):
        ...
        if (not self.pointsChosen):
            ...
            self.moveMiddleMenuOut(self.moveSidesToStartGame)
    ...
    def qs1Click(self,event):
        ...
        if (self.qs1Clicked):
            ...
            self.startPulsating(self.qs2, 0.3)
            
            if (self.qs2Clicked):
                self.qs2.pulsate = False
                self.unlinkAll()
                self.musicPlayer.stop()
                self.main.initGame(self.parentNode, self.highscore, -1, 1) 
    
    
    def startPulsating(self, node, opacity):
        if (node.pulsate):
            oldOpacity = node.opacity
            avg.LinearAnim(node, "opacity", 1500, oldOpacity, opacity, None, None, lambda : self.startPulsating(node, oldOpacity)).start();  
        else:
            node.opacity = 1.0
    ...
    def qs2Click(self,event):
        ...
        self.soundPlayer.playTune(buttonSound)
        if (self.qs2Clicked):
            self.qs1.pulsate = False
            self.qs2Clicked = False
            self.qs2.opacity = 1.0
        else:
            self.qs2Clicked = True
            self.qs2.opacity = 0.5
            self.qs1.pulsate = True
            self.startPulsating(self.qs1, 0.3)
            if (self.qs1Clicked):
                self.qs1.pulsate = False
                self.unlinkAll()
                self.musicPlayer.stop()
    ...
    def showScoreMenu(self):  
        ...
        """
        self.back.setEventHandler(avg.CURSORDOWN, avg.TOUCH or avg.MOUSE, self.fromScoreMenu)
        self.letBackButtonAppear()
...
######################  HELP MENU  ############################# 
    ...
    def showHelpMenu(self):
        ...
        self.dragDiv.setEventHandler(avg.CURSORDOWN,avg.TOUCH or avg.MOUSE, self._startDragging)
        self.dragDiv.setEventHandler(avg.CURSORMOTION,avg.TOUCH or avg.MOUSE, self._doDragging)
        self.dragDiv.setEventHandler(avg.CURSORUP,avg.TOUCH or avg.MOUSE, self._endDragging)
