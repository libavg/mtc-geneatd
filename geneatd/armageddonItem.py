...
class ArmageddonItem(Item):
    """
    This class represents an armageddon item. 
    When player drags this item into his base, he destroys all creatures on the field.
    """
    ...
    def setAppearance(self):
        ...
        """
        self.node.filltexhref=os.path.join(getMediaDir(__file__, "resources"), 'biogold.png')    
    ...
    def _executeItemEffect(self, team):
        ...
        """
        self.game.soundPlayer.playTune('soundfiles/sound_item_armageddon.mp3')
        
        c1list = Creature.creatures1[:]
        c2list = Creature.creatures2[:]
       
        for creature in c1list:
            creature.die()
          
        for creature in c2list:
            creature.die()
        
        team.adjustExp(self.exp)
        team.adjustScore(300)
    ...
    def _executeItemGraphicsEffect(self, team):
        """
        This method should be overwritten for effect.
        Should contain the effect an item has on the GUI when collected.
        """
        self.lightning = avg.RectNode(fillopacity=0.01, fillcolor="FFFFFF", sensitive=False, size=util.playingFieldSize, pos=util.playingFieldPos, strokewidth=0, parent=self.graphicsLayer)
